...
export class AbsenceNoticesService {
    ...
    async create(dto: CreateAbsenceNoticeDto, parentId: string) {
        ...
        if (!athlete) {
            throw new NotFoundException('Atleta não encontrado ou não associado a este encarregado de educação');
        }
        ...
        try {
            ...
            console.error('Failed to send notifications for absence notice', error);
