...
export class AthletesService {
    ...
    async requestTransfer(clubId: string, publicId: string) {
        ...
        if (athlete.currentClubId) {
            throw new BadRequestException('Athlete is currently active in another club. Withdrawal required first.');
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            const player = await tx.player.create({
                data: {
                    ...
                    parentId: await this.findOrCreateUserForClub(tx, request.toClubId, request.athlete.globalParentId),
                }
            });
    ...
    private async findOrCreateUserForClub(tx: any, clubId: string, globalParentId: string): Promise<string> {
    ...
    async getCurrentClub(athleteId: string) {
        ...
        if (player.status === 'PENDING_WITHDRAWAL' && player.withdrawalRequestedAt) {
            ...
            const hoursRemaining = Math.max(0, Math.floor((coolOffEnds.getTime() - Date.now()) / (1000 * 60 * 60)));
