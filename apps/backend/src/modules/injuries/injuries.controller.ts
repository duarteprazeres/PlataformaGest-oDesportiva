...
export class InjuriesController {
    ...
    @Get()
    findAll(@Request() req: RequestWithUser, @Query('activeOnly') activeOnly?: string, @Query('playerId') playerId?: string) {
    ...
    @Patch(':id')
    update(@Request() req: RequestWithUser, @Param('id') id: string, @Body() updateInjuryDto: UpdateInjuryDto) {
