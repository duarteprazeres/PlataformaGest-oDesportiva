...
export class MailService {
    ...
    async sendEmail(to: string, subject: string, html: string, attachments?: {
        filename: string;
    ...
    async sendWithdrawalPackage(to: string, athleteName: string, clubName: string, attachments: { filename: string, content: Buffer | string }[]) {
    ...
    `;
        ...
        const formattedAttachments = attachments.map(att => {
            if (typeof att.content === 'string' && (att.content.startsWith('http') || att.content.startsWith('/'))) {
                return {
