...
export class TrainingsService {
  ...
  async create(
    ...
    const endDate =
      createTrainingDto.isRecurring && createTrainingDto.recurrenceEndDate
  ...
  async findAll(clubId: string, teamId?: string, status?: 'upcoming' | 'pending_lock' | 'history') {
    const now = new Date();
  ...
  async update(
    ...
    const { isRecurring, frequency, recurrenceEndDate, ...activeData } = updateTrainingDto;

    // Create a new object for Prisma update to avoid 'any'
    const data: Prisma.TrainingUpdateInput = {
      ...activeData,
      ...(planFileUrl ? { planFileUrl } : {}),
      scheduledDate: activeData.scheduledDate ? new Date(activeData.scheduledDate) : undefined,
    };
  ...
  async markAttendance(
    ...
    const injuredOrSickPlayers = teamPlayers.filter(
      (p) => p.medicalStatus === 'INJURED' || p.medicalStatus === 'SICK',
    );
    ...
    for (const att of markAttendanceDto.attendance) {
      ...
      if (!player) {
        throw new BadRequestException(`Jogador ${att.playerId} não pertence à equipa deste treino`);
      }
